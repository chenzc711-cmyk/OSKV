const STORAGE_KEY = "tmallWarehouseV2";
...
function renderInventory() {
  ...
  body.innerHTML = filteredProducts("inventorySearch").map((product) => `
    <tr>
      ...
      <td><span class="badge ${product.stock <= product.warning ? "low" : "ok"}">${product.stock <= product.warning ? "库存不足" : "库存正常"}</span></td>
      <td><button class="link-btn" data-adjust="${product.id}">✎ 调整</button><button class="link-btn" data-detail="${product.id}">详情</button></td>
    </tr>`).join("");
}
...
function renderProducts() {
  ...
  document.getElementById("productBody").innerHTML = rows.map((product) => `
    <tr>
      <td><strong>${product.name}</strong></td><td>${product.category}</td><td>${product.unit}</td><td class="money">${money(product.price)}</td>
      <td><span class="badge ${product.stock <= product.warning ? "low" : "ok"}">${product.stock}</span></td><td><span class="badge ok">启用</span></td>
      <td><button class="link-btn" data-cost-product="${product.id}">$</button><button class="link-btn" data-edit-product="${product.id}">✎</button><button class="delete-btn" data-delete-product="${product.id}">▢</button></td>
    </tr>`).join("");
}
